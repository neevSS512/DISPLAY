...
const SnakeScoreData = () => {
    ...
    try {
      ...
      await Promise.all(
        newRows.map(async (newRow) => {
          ...
          const response = await axios.post('http://147.93.27.170:3001/snakepublic/snakepublicctg', newRow);
  ...
  useEffect(() => {
    const fetchGameuserData = async () => {
      try {
        const response = await axios.get('http://147.93.27.170:3001/snakepublic/snakepublicctg');
  ...
  const updateData = async (item) => {
    try {
      ...
      const response = await axios.patch(`http://147.93.27.170:3001/snakepublic/snakepublicctg/${item._id}`, updatedUser);
...
const handleDelete = async (id) => {
  ...
  if (isConfirmed) {
    try {
      ...
      const response = await axios.delete(`http://147.93.27.170:3001/snakepublic/snakepublicctg/${id}`);
